...
class CodeformerDataset(data.Dataset):
    ...
    def __init__(
        ...
        # self.mask_file = load_file_list(mask_list)  ###NOTE
        # self.datasets = dataset_registry[dataset]()
        # self.image_files, self.mask_files = self.load_img_gt_list(self.datasets)
        self.image_files, self.mask_files = dataset['img'], dataset['gt']
    ...
    def load_gt_image(self, image_path: str, max_retry: int=5) -> Optional[np.ndarray]:
        ...
        if self.crop_type != "none":
            ...
            image = image.resize((self.out_size, self.out_size), resample=Image.BICUBIC)  # added by guo
    ...
    def get_mask(self, mask_path):
        ...
        ori_mask = torch.tensor(mask, dtype=torch.uint8)/255.0
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[np.ndarray, str]]:
        ...
        # load masks --added by guo
        mask_path = self.mask_files[index]
        # mask_path = mask_files["image_path"]
        mask, ori_mask = self.get_mask(mask_path)
        input_point, input_label = self.get_prompt(mask, 3)  # 3 points  
        ...
        mode = random.choice([cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA])
        # 先下采样，在乘以对应的倍率
        ds_scale = random.choice(self.opt['scale'])
        out = cv2.resize(out, (int(w / ds_scale * scale), int(h / ds_scale * scale)), interpolation=mode)
        ...
        # return gt, lq, mask, input_point, input_label
        return data_dict
