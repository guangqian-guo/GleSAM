...
parser.add_argument("--sf", type=int, default=4, help='scale factor, 1, 2, 4.')
...
train_dataset = SimpleDataset(args_training_dataset, fix_size=1024)
...
def realesrgan_degradation(batch,  args_degradation, use_usm=True, sf=4, resize_lq=True):
    ...
    # im_gt = F.interpolate(im_gt, size=(1024, 1024), mode='bilinear')
...
gt_path = os.path.join(root_path, 'ori')
...
with torch.no_grad():
    for epoch in range(epochs):
        for num_batch, batch in enumerate(tqdm(train_dataloader)):
            sf = 2
            lr_batch, gt_batch = realesrgan_degradation(batch, args_degradation=args_degradation, sf=sf)
            ...
            for i in range(batch_size):
                ...
                img_name = batch['lq_path'][i].split('/')[-1]
                lr_save_path =  os.path.join(lr_path,img_name)
                gt_save_path =  os.path.join(gt_path, img_name)
                sr_bicubic_save_path =  os.path.join(sr_bicubic_path, img_name)
