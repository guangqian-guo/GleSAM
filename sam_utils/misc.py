...
def box_noise(boxes, box_noise_scale=0.05):
    # print(boxes)
...
def gen_noise_gt(boxes, box_noise_level=0.1):
    noisy_bboxes = []
    print(boxes)
    ...
    noisy_bboxes = np.array(noisy_bboxes).astype(np.float32)
    print(noisy_bboxes)
    return noisy_bboxes
...
def masks_sample_pos_neg_points(masks,k=1):
    """Sample points on mask
    """
    if masks.numel() == 0:
        return torch.zeros((0, 2), device=masks.device)
    ...
    for b_i in range(len(masks)):
        ...
        samples_x = x_idx[idx]
        samples_y = y_idx[idx]
        pos_samples_xy = torch.cat((samples_x[:, None], samples_y[:, None]), dim=1)
        ...
        samples_x = x_idx[idx]
        samples_y = y_idx[idx]
        neg_samples_xy = torch.cat((samples_x[:, None], samples_y[:, None]), dim=1)
        samples_xy = torch.cat((pos_samples_xy, neg_samples_xy), dim=0)
