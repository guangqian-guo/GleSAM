...
def get_im_gt_name_dict(datasets, flag='valid'):
    ...
    for i in range(len(datasets)):
        ...
        try:
            tmp_edge_list = [datasets[i]["edge_dir"]+os.sep+x.split(os.sep)[-1].split(datasets[i]["im_ext"])[0]+datasets[i]["gt_ext"] for x in tmp_im_list]
            print('-edge-', datasets[i]["name"], datasets[i]["edge_dir"], ': ',len(tmp_gt_list))
        except:
            tmp_edge_list = None
        
        if tmp_edge_list is not None:
...
def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=False):
    ...
    if training:
        for i in range(len(name_im_gt_list)):   
            gos_dataset = OnlineDataset_edge([name_im_gt_list[i]], transform = transforms.Compose(my_transforms))
...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # NOTE bug on A100 !!!!!  不加这个，有可能是0-1的二值图  --2024/07/11 
        if gt.max() == 1.:
            gt[gt == 1.]  = 255.
        ####################################
...
class OnlineDataset_edge(Dataset):
    def __init__(self, name_im_gt_list, transform=None, eval_ori_resolution=False):
        ...
        for i in range(0,len(name_im_gt_list)):
            ...
            im_ext_list.extend([name_im_gt_list[i]["im_ext"] for x in name_im_gt_list[i]["im_path"]])
            gt_ext_list.extend([name_im_gt_list[i]["gt_ext"] for x in name_im_gt_list[i]["gt_path"]])
            edge_ext_list.extend([name_im_gt_list[i]["edge_ext"] for x in name_im_gt_list[i]["edge_path"]])
        ...
        self.dataset["ori_edge_path"] = deepcopy(edge_path_list)
        self.dataset["im_ext"] = im_ext_list
        self.dataset["gt_ext"] = gt_ext_list
        self.dataset["edge_ext"] = edge_ext_list
    ...
    def __getitem__(self, idx):
        im_path = self.dataset["im_path"][idx]
        gt_path = self.dataset["gt_path"][idx]
        edge_path = self.dataset["edge_path"][idx]
        ...
        im = torch.transpose(torch.transpose(im,1,2),0,1)
        gt = torch.unsqueeze(torch.tensor(gt, dtype=torch.float32),0)
         # NOTE BUG   版本问题， 不加这个，有可能是0-1的二值图  --2024/07/11 
        if gt.max() == 1.:
            gt[gt == 1.]  = 255.
        ####################################
        ...
        # print(sample['label'].shape, sample['edge'].shape)
        # show 
        # import matplotlib.pyplot as plt
        # plt.subplot(1, 2, 1)
        # plt.imshow(sample['label'].squeeze(0), cmap='gray')
        # plt.title('gt')
        # plt.subplot(1,2,2)
        # plt.imshow(sample['edge'].squeeze(0), cmap='gray')
        # plt.title('edge')
        # plt.show()
